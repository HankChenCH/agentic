...
def repair(engine, vector_index, *, apply: bool = False) -> RepairReport:
    ...
    with Session(engine) as session:
        ...
        # ---- ❻ 定向向量重写（touched 实体 + 改挂 statement）----
        entries = [
        ...
        vector_index.upsert_many(entries)
        report.log("修复", f"向量重写 {len(entries)} 条（{KIND_ENTITY}"
...
def collect_index_entries(engine) -> tuple[list[VectorEntry], dict[str, int]]:
    """收集全量向量条目与分类计数（rebuild-index 的探测与执行共用）。
...
def rebuild_index(engine, vector_index) -> dict[str, int]:
    """drop Memory collection 后按收集结果全量重嵌，返回分类计数。"""
...
def rebuild_index_cmd(
    ...
    if not yes:
        ...
        typer.echo(f"将 drop Memory collection 并重嵌 {len(entries)} 条（{detail}）")
        typer.echo("确认后加 --yes 执行")
        return
    vector_index.rebuild(entries)
    typer.echo(f"== 记忆向量索引重建完成：{len(entries)} 条（{detail}）==")
