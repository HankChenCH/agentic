...
class MemoryConsolidationService:
    ...
    def _resolve_entities(self, candidates: tuple[ExtractedEntity, ...]) -> dict[str, MemoryEntity]:
        ...
        for candidate in candidates:
            if renderer.is_internal_ref(candidate.name):
                ...
                continue
            hit = (
                resolution.exact_entity_match(candidate.name, self.memory_repo)
                or next(
            ...
            if hit is None:
                hit = resolution.vector_entity_match(
                    candidate.name, candidate.entity_type, self.memory_repo,
                    self.vector_index, threshold,
                )
    ...
    def _apply_decision(self, pair: PairedFact, decision: FactDecision, by_id, live_subject, now) -> MemoryStatement | None:
