...
def test_remember_adds_statement_with_evidence_and_vector_entry(engine):
    llm = CannedLLM(_fact_response("后端开发"), '[{"index": 0, "action": "ADD"}]')
    svc = _service(engine, llm)
    turn_id = uuid4()

    created = svc.remember(query="我在上海做后端开发", turn_messages=[_msg()],
...
def test_single_value_predicate_replaces_old_via_llm_decision(engine):
    seed_llm = CannedLLM(_fact_response("教师"), '[{"index":0,"action":"ADD"}]')
    seed_svc = _service(engine, seed_llm)
    seed_svc.remember("我是教师", [_msg()], uuid4(), uuid4())
    old_row = seed_svc.memory_repo.list_active_statements()[0]

    llm = CannedLLM(_fact_response("后端开发"),
                    json.dumps([{"index": 0, "action": "REPLACE", "replace_id": old_row.id}],
                               ensure_ascii=False))
    created = _service(engine, llm).remember("我改行做后端了", [_msg()], uuid4(), uuid4())

    active = created[0].__class__ and [r for r in
        SqliteGraphMemoryRepository(engine=engine).list_active_statements()]
    assert len(active) == 1 and active[0].object_text == "后端开发" and active[0].id != old_row.id

    reloaded_old = SqliteGraphMemoryRepository(engine=engine).statements_by_ids([old_row.id])[0]
...
def test_manual_origin_cannot_be_superseded(engine):
    ...
    manual = svc.memory_repo.insert_statement(MemoryStatement(
        subject_id=svc._ensure_user_entity().id,
    ...
    conflict_llm = CannedLLM(_fact_response("程序员"),
                             json.dumps([{"index": 0, "action": "REPLACE", "replace_id": manual.id}]))
    result = _service(engine, conflict_llm).remember("我现在是程序员", [_msg()], uuid4(), uuid4())
...
def test_programmatic_fallback_when_adjudication_fails(engine):
    ...
    done = _service(engine, fallback_llm).remember("转岗成测试工程师了", [_msg()], uuid4(), uuid4())

    states = {row.id: row.state for row in
              SqliteGraphMemoryRepository(engine=engine).statements_by_ids([old_id])}
