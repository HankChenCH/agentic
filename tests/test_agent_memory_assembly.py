"""agent 层快速记忆装配：BaseAgent._input 的 system prompt 组装与 RAG 折叠。

BaseAgent 默认把快注块并入头部 SystemMessage（人设 + 块合并为一条，避免双
system 消息的提供商兼容问题）；RAG 因图节点按文本渲染历史（understand 会
丢弃 SystemMessage），在其 _input 折进末条用户消息。检索失败一律降级不阻断。
图不参与本组测试——桩子类的 build_graph 返回 None，仅验证输入装配。
"""
...
def test_input_composes_block_into_system_message():
    """块存在时并入头部 SystemMessage：人设 + 块一条消息，用户消息保持纯净。"""
    recall = FakeRecall(block=BLOCK)
    agent = _StubAgent(model=None, toolbox=_toolbox(recall))
    history = AIMessage(content="先前回答")
    ctx = _ctx("我上周聊到什么了？", history=[history])

    messages = agent._input(ctx)["messages"]

    assert isinstance(messages[0], SystemMessage)
    assert messages[0].content == f"人设\n\n{BLOCK}"
    # 历史原样回放；末条只注入时间前缀，原始 query 不被改写
    assert messages[1] is history
    assert messages[-1].content == f"[当前时间：{NOW}]\n\n我上周聊到什么了？"
    assert "用户提问：" not in messages[-1].content
    # 快注以原始 query 触发（寒暄短路判别），身份转为 UUID 透传
    assert recall.queries == [("我上周聊到什么了？", UUID(TEST_USER), UUID(THREAD))]


def test_input_without_block_keeps_bare_persona():
    """空块（寒暄/未启用/无记忆）：system prompt 为裸人设。"""
    agent = _StubAgent(model=None, toolbox=_toolbox(FakeRecall(block="")))

    messages = agent._input(_ctx("hi"))["messages"]

    assert isinstance(messages[0], SystemMessage) and messages[0].content == "人设"


def test_input_degrades_to_persona_when_recall_fails():
    """快注失败降级为仅人设：warning 不阻断，主链路照常。"""
