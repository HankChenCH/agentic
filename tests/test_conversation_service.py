...
def test_describe_and_delete_raise_when_missing(service):
    ...
    with pytest.raises(ConversationNotFoundError):
        service.describe_conversation(thread_id=missing)
    with pytest.raises(ConversationNotFoundError):
        service.delete_conversation(thread_id=missing)


def test_open_turn_persists_conversation_turn_and_user_message(service, engine):
    thread_id = uuid4()
    conversation, turn = service.open_turn(thread_id=thread_id, run_id="run-1", query="你好")
...
def test_record_turn_usage_accumulates_with_whitelist(service, engine):
    _, turn = service.open_turn(thread_id=uuid4(), run_id="run-2", query="你好")
...
def test_management_envelopes_empty_state(service):
    assert service.list_conversations(page=1, page_size=10) == {"items": [], "total": 0, "page": 1, "pageSize": 10}
    assert service.list_history_messages(thread_id=uuid4())["items"] == []
