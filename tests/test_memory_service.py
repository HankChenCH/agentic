...
def test_smalltalk_and_disabled_short_circuit(engine):
    ...
    assert svc.build_fast_context("你好呀", THREAD) == ""
    assert _service(engine, enabled=False).build_fast_context("我在哪工作", THREAD) == ""
...
def test_fast_context_damps_cold_items_and_bumps_accessed(engine):
    ...
    block = svc.build_fast_context("推荐个编程语言", THREAD)
...
def test_session_registry_returns_only_increment_across_calls(engine):
    ...
    first_ids = _rendered_ids(svc.build_fast_context("聊点爱好", THREAD))
    second_ids = _rendered_ids(svc.build_fast_context("还有别的吗", THREAD))
...
def test_disabled_engine_yields_empty_even_with_rows(engine):
    repo = SqliteGraphMemoryRepository(engine=engine)
    _seed(repo, "Python")
    assert _service(engine, enabled=False).build_fast_context("任何问题", THREAD) == ""
